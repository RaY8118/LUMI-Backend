...
def recognize_face(encoding_to_check):
    ...
    if matches[best_match_index]:
        identified_name = studentIds[best_match_index]
...
def process_image(image_file):
    new_image = cv2.imdecode(np.fromstring(
    ...
    for (top, right, bottom, left) in face_locations:
        name = recognize_face(face_recognition.face_encodings(
            rgb_image, [(top, right, bottom, left)])[0])
